...
<div class="container py-4 py-md-5 booking-page">
    <div class="booking-public-shell">
        ...
        @if($customerAppointments->isNotEmpty())
            <section class="booking-subscriptions booking-card">
                <div class="booking-subscription-heading"><div><span>Seus próximos horários</span><h2>Agendamentos</h2></div></div>
        ...
        <section class="booking-card">
            <form method="GET" action="{{ route('service-booking.book', $ad) }}" class="booking-selection-form">
                ...
                @if($procedure)
                    <div class="booking-procedure-details">
                        <strong>{{ $procedure->name }}</strong>
                        @if($procedure->description)<p>{{ $procedure->description }}</p>@else<p>Consulte o profissional para confirmar tudo o que está incluído neste procedimento.</p>@endif
                        <span>R$ {{ number_format($procedure->price, 2, ',', '.') }} · duração aproximada de {{ $procedure->duration_minutes }} minutos</span>
                    </div>
                @endif
                <label>Profissional
            ...
            @if($procedure && $staff)
                <form method="POST" action="{{ $rescheduleAppointment ? route('service-booking.customer.reschedule', [$ad, $rescheduleAppointment]) : route('service-booking.store', $ad) }}" class="booking-confirm-form">
                    @csrf
